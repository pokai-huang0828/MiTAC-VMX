...
- **Q1 passenger-only**：redocly v2026-Q1 沒 blurring API；Jira [HAWK-331](https://jira.navman.co.nz/jira/browse/HAWK-331) / 527 / 578 全是 all-face；**現行不支援駕乘區分**；BMS DMS 結果可餵 blur 模組是可行路徑，但需新 ticket
...
_Earlier 2026/05/07：補入 Cary 5/7 11:22 新提兩個問題（passenger-only / fleet-level）+ Teams session 請求。對照 [HAWK-527](https://jira.navman.co.nz/jira/browse/HAWK-527) description 發現 driver_blurry/passenger_blurry 已是分開 flag，redocly 沒文件化（→ memory `feedback_jira_first_not_redocly.md`）。_
...
- 對應 [HAWK-527](https://jira.navman.co.nz/jira/browse/HAWK-527) description 中 driver_blurry / passenger_blurry 分開 flag 的觀察 ✓
...
- [VMX-6427](https://jira.navman.co.nz/jira/browse/VMX-6427) ≠ Blurring monthly report 的對應位置 — overlap 但不是同一件事
- **決策(2026-05-11)**:Blurring monthly report 等 Production deploy 後(end of July 後)有具體 SI usage 資料時再開新單,不疊在 [VMX-6427](https://jira.navman.co.nz/jira/browse/VMX-6427) 上
- **對 Cary 口徑**:「Tracked as future enhancement post-Q2 production, will scope once we have real usage data」

#### Kenny 在 [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) / 7458 的 2 個 sharp 處理
1. **#1+#2 合併在 [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457)**:cloud integration + API doc share 同票,doc share 寫進 AC,不分兩張避免並行追蹤負擔
2. **[VMX-7458](https://jira.navman.co.nz/jira/browse/VMX-7458) 把 pattern reference 改精準**:從「Live User」改成「**Driver-Facing Camera Live View**」— 對 Lucy / GUI team 更明確

#### [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) 內 2 個 deliverable 不同 timeline(Kenny 5/11 加 comment 釐清)

合併同票會讓票面 Due date(30/Jun/26)誤導 — 看似 doc share 也要等到 6 月底。**Kenny 在 [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) 加 comment 標清楚 2 個 deliverable 不同 commitment**:
...
→ **Comment 已加進 [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457)**,Brian / Spencer 一讀就知道兩條 timeline 不同 · 後續對 Cary 任何承諾都 traceable 在 ticket 上。
...
- [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) / [VMX-7458](https://jira.navman.co.nz/jira/browse/VMX-7458) ticket link
...
- Jira tickets [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) / [VMX-7458](https://jira.navman.co.nz/jira/browse/VMX-7458)
...
| 等於 Kenny 動作被 gate | **Brian 對 Kenny 的開單質量肯定**(特別是 [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) 拆 Deliverable A/B 那個 comment 動作有效) |
...
| "monthly subscription report?" | "Tracked as future enhancement post-Q2 production. Data foundation is in [VMX-7457](https://jira.navman.co.nz/jira/browse/VMX-7457) (every API call logged). We'll scope the format with you once we have real usage data flowing — happy to revisit format after July." |
