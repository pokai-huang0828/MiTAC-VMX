...
Cary / Wendy 線(MiTAC AU)推的 passenger blurring 不是 one-off,而是「**跨客戶 reusable 隱私 / detection feature**」族的一個。本週 HAWK / VMX 看到的對應案:

| Jira | Title | 跟 Cary 線的對應 |
|------|-------|----------------|
| [HAWK-582](https://jira.navman.co.nz/jira/browse/HAWK-582) | Improve Lens cover detection design | **同 pattern**:Webfleet (Sebastian Schneider) 報告 + Bridgestone (Jakob Mund) + Azuga(Sirphi Manivannan)三客戶聯動,要改 detection 觸發條件(取消 speed ≥ 20 km/h + DMS calibration 完成限制)。**Eric H 5/8 13:38 確認 6 月 release**。Cary 線後續可能 ask 同一個改動 |
| [HAWK-502](https://jira.navman.co.nz/jira/browse/HAWK-502) | Changes to the privacy mode | **直接相關** — Private Mode 改動,可能影響 Cary 端 passenger blurring 的隱私語意對應 |
| [VMX-5908](https://jira.navman.co.nz/jira/browse/VMX-5908) | Contract fleet 的 Webhook 處理 | **Q2 portal path 對應**:這是 Jira 上「per-contract-fleet 議題」的位置之一 |
| [VMX-6586](https://jira.navman.co.nz/jira/browse/VMX-6586) | Group main fleet and contract fleets together | **Q2 portal path 對應**:另一個對應票,Kenny 可建議升優先級 |

### 戰略觀察(Kenny 5/11 思考)

- Cary / Wendy 線推的「跨客戶 reusable feature」其實 RD 端已經在改(HAWK-582 確認 6 月);**Lens Cover、Privacy Mode、Passenger Blurring 三者可以包成一個「Privacy & Detection Triggering」族對外溝通**
- per-contract-fleet config 議題已經有兩張 VMX ticket pending 180+ 天 — 不是新需求,是舊需求一直沒被排優先級
- Kenny 下一步:**主動串 HAWK-582 6 月 release 消息給 Cary / Wendy**,當作「passenger blurring 同 pattern 有先例 + 我們在 ship」的對外證據

_2026-05-11 update:加 I 段 平行案族 + 戰略觀察。_
